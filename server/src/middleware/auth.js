...
const auth = async (req, res, next) => {
    ...
    if (authHeader) {
        ...
        try {
            if (!token) {
                return res.status(401).json({ message: 'ไม่มี Token ในการ login ถูกสร้างมากรุณา Login ใหม่เพื่อสร้าง Token!' });
            }
            ...
            if (blacklist.has(token.trim())) {
                return res.status(401).json({ message: 'ไม่สามารถใข้งานระบบได้กรุณา Login ใหม่!' });
            }
